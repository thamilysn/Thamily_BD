...
# Nesta tabela pagamentos tem a FK de aluno_id, então daqui para baixo a ordem vai ficar entre tabelas com FK's e relações associativas
...
# Como a tabela de aulas depende de um atributo que está na associação de instrutor com modalidades, a criei primeiro:
...
CREATE TABLE IF NOT EXISTS PLANO_TREINAMENTO (
	...
	CONSTRAINT PLANO_INSTMOD_FK FOREIGN KEY (PT_INSTMOD_INST_ID) REFERENCES INST_MOD (INSTMOD_INST_ID),
	CONSTRAINT PLANO_MODINST_FK FOREIGN KEY (PT_INSTMOD_MOD_ID) REFERENCES INST_MOD (INSTMOD_MOD_ID)
);
